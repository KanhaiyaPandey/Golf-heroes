...
interface Charity {
  ...
  imageUrl: string | null; websiteUrl: string | null;
...
export default function CharitiesPage() {
  ...
  const [form, setForm] = useState({ name: "", slug: "", description: "", imageUrl: "", websiteUrl: "", isFeatured: false });
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ name: "", slug: "", description: "", imageUrl: "", websiteUrl: "", isFeatured: false });
      void load();
    ...
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-zinc-400">{charities.length} charities registered</p>
        </div>
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {[
              ...
              <div key={name}>
                ...
                <input required={required} value={(form as Record<string,string>)[name]} onChange={(e) => setForm({ ...form, [name]: e.target.value })}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {loading ? (
          <div className="col-span-3 py-16 text-center text-zinc-500">Loading…</div>
        ) : charities.map((c) => (
          <div key={c.id} className={`rounded-2xl border bg-zinc-900/60 overflow-hidden ${c.isActive ? "border-zinc-800" : "border-zinc-800 opacity-60"}`}>
            <div className="relative h-36 bg-zinc-800">
              ...
              <div className="absolute top-2 right-2 flex gap-1.5">
                {c.isFeatured && <span className="rounded-full bg-amber-500/80 px-2 py-0.5 text-xs font-medium text-white">Featured</span>}
                {!c.isActive && <span className="rounded-full bg-red-500/80 px-2 py-0.5 text-xs font-medium text-white">Inactive</span>}
              </div>
            </div>
            <div className="p-4">
              ...
              <div className="mt-3 flex gap-2">
                <button onClick={() => toggleFeatured(c.id, c.isFeatured)}
